...
class SharedFileTag:
    ...
    def from_subtags(subtags: List[Tag]) -> "SharedFileTag":
        ...
        size_tag = num(ECTagName.EC_TAG_PARTFILE_SIZE_FULL)
        ...
        return SharedFileTag(
            ...
            up_prio=_to_signed_byte(
                find_byte(subtags, ECTagName.EC_TAG_KNOWNFILE_PRIO).get_value()
            ),
            get_requests=num(ECTagName.EC_TAG_KNOWNFILE_REQ_COUNT).get_short(),
            get_all_requests=num(ECTagName.EC_TAG_KNOWNFILE_REQ_COUNT_ALL).get_int(),
            get_accepts=num(ECTagName.EC_TAG_KNOWNFILE_ACCEPT_COUNT).get_short(),
            get_all_accepts=num(ECTagName.EC_TAG_KNOWNFILE_ACCEPT_COUNT_ALL).get_int(),
            get_xferred=num(ECTagName.EC_TAG_KNOWNFILE_XFERRED).get_long(),
            get_all_xferred=num(ECTagName.EC_TAG_KNOWNFILE_XFERRED_ALL).get_long(),
            get_complete_sources_low=num(
                ECTagName.EC_TAG_KNOWNFILE_COMPLETE_SOURCES_LOW
            ).get_short(),
            get_complete_sources_high=num(
                ECTagName.EC_TAG_KNOWNFILE_COMPLETE_SOURCES_HIGH
            ).get_short(),
            get_complete_sources=num(
                ECTagName.EC_TAG_KNOWNFILE_COMPLETE_SOURCES
            ).get_short(),
            get_on_queue=num(ECTagName.EC_TAG_KNOWNFILE_ON_QUEUE).get_short(),
    ...
    def from_subtags(subtags: List[Tag]) -> "PartFileTag":
        ...
        return PartFileTag(
            ...
            file_status=FileStatus.from_value(
                find_byte(subtags, ECTagName.EC_TAG_PARTFILE_STATUS).get_value()
            ),
            stopped=find_byte(subtags, ECTagName.EC_TAG_PARTFILE_STOPPED).get_value()
            != 0,
            source_count=num(ECTagName.EC_TAG_PARTFILE_SOURCE_COUNT).get_short(),
            source_not_curr_count=num(
                ECTagName.EC_TAG_PARTFILE_SOURCE_COUNT_NOT_CURRENT
            ).get_short(),
            source_xfer_count=num(
                ECTagName.EC_TAG_PARTFILE_SOURCE_COUNT_XFER
            ).get_short(),
            source_count_a4af=num(
                ECTagName.EC_TAG_PARTFILE_SOURCE_COUNT_A4AF
            ).get_short(),
            speed=speed.get_long() if speed is not None else None,
            down_prio=_to_signed_byte(
                find_byte(subtags, ECTagName.EC_TAG_PARTFILE_PRIO).get_value()
            ),
            file_cat=num(ECTagName.EC_TAG_PARTFILE_CAT).get_long(),
            last_seen_complete=num(ECTagName.EC_TAG_PARTFILE_LAST_SEEN_COMP).get_long(),
            last_date_changed=num(ECTagName.EC_TAG_PARTFILE_LAST_RECV).get_long(),
            download_active_time=num(
                ECTagName.EC_TAG_PARTFILE_DOWNLOAD_ACTIVE
            ).get_int(),
            available_part_count=num(
                ECTagName.EC_TAG_PARTFILE_AVAILABLE_PARTS
            ).get_short(),
            a4af_auto=find_byte(
                subtags, ECTagName.EC_TAG_PARTFILE_A4AFAUTO
            ).get_value()
            != 0,
            hashing_progress=find_byte(
                subtags, ECTagName.EC_TAG_PARTFILE_HASHED_PART_COUNT
            ).get_value()
            != 0,
            get_lost_due_to_corruption=num(
                ECTagName.EC_TAG_PARTFILE_LOST_CORRUPTION
            ).get_long(),
            get_gain_due_to_compression=num(
                ECTagName.EC_TAG_PARTFILE_GAINED_COMPRESSION
            ).get_long(),
            total_packets_saved_due_to_ich=num(
                ECTagName.EC_TAG_PARTFILE_SAVED_ICH
            ).get_int(),
        )
